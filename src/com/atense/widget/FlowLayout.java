...
/**
 * FlowLayout will arrange child elements horizontally one next to another. If
 * there is not enough space for next view new line will be added.
 *
 * @author Kai.Li
 */
...
public class FlowLayout extends ViewGroup {
	...
	private int mGravity = (isIcs() ? Gravity.START : Gravity.LEFT)
			| Gravity.TOP;
	/*
	 * 最大的列数
	 */
	...
	/*
	 * 每列的宽度，列与列之间的宽度为0 ，也就是说不控制每列之间的间隔，每列之间的间隔用还是的Margin来控制
	 */
	...
	public FlowLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		try {
			...
			this.columns = a.getInt(R.styleable.FlowLayout_columns, 1);
		} finally {
			a.recycle();
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		this.itemWidth = sizeWidth / this.columns;
		...
		for (int i = 0; i < childCount; i++) {
			...
			int h = View.MeasureSpec.makeMeasureSpec(0,
					View.MeasureSpec.UNSPECIFIED);
			child.measure(w, h);
			int aWidth = child.getMeasuredWidth();
			...
			if (lp.width == LayoutParams.MATCH_PARENT
				...
				if (itemWidth < aWidth) {
					int count = aWidth % itemWidth > 0 ? aWidth / itemWidth + 1
							: aWidth / itemWidth;
					if (count > this.columns) {
						childWidthSize = sizeWidth - lp.leftMargin
								- lp.rightMargin;
					} else {
						childWidthSize = count * itemWidth - lp.leftMargin
								- lp.rightMargin;
					}
				} else {
					childWidthSize = itemWidth - lp.leftMargin - lp.rightMargin;
				}
			} else if (lp.width >= 0) {
				childWidthMode = MeasureSpec.EXACTLY;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (lineWidth + childWidth > width) {
				...
				mLineMargins
						.add((int) ((width - lineWidth) * horizontalGravityFactor)
								+ getPaddingLeft());
		...
		for (int i = 0; i < numLines; i++) {
			...
			for (int j = 0; j < children; j++) {
				...
				// if height is match_parent we need to remeasure child to line
				// height
				if (lp.height == LayoutParams.MATCH_PARENT) {
					...
					child.measure(MeasureSpec.makeMeasureSpec(childWidthSize,
							childWidthMode), MeasureSpec.makeMeasureSpec(
							lineHeight - lp.topMargin - lp.bottomMargin,
							MeasureSpec.EXACTLY));
				}
				...
				if (Gravity.isVertical(lp.gravity)) {
					...
					case Gravity.CENTER_VERTICAL:
					case Gravity.CENTER:
						gravityMargin = (lineHeight - childHeight
				...
				child.layout(left + lp.leftMargin, top + lp.topMargin
						+ gravityMargin + verticalGravityMargin, left
						+ childWidth + lp.leftMargin, top + childHeight
						+ lp.topMargin + gravityMargin + verticalGravityMargin);
	...
	/**
	 * @return <code>true</code> if device is running ICS or grater version of
	 *         Android.
	 */
	...
	public static class LayoutParams extends MarginLayoutParams {
		...
		public LayoutParams(Context c, AttributeSet attrs) {
			...
			try {
				gravity = a.getInt(
						R.styleable.FlowLayout_Layout_android_layout_gravity,
						-1);
			} finally {
				a.recycle();
			}
